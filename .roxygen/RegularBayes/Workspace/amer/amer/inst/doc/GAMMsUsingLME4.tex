...
\author{Fabian Scheipl\footnote{\texttt{fabian.scheipl@stat.uni-muenchen.de}} \\ LMU München}
\institution
...
\subsection{Reparameterization: Separating penalized and unpenalized function components}\label{Reparam}
...
\end{eqnarray*}
The reformulation of the additive model as a mixed model therfore makes it possible to estimate
...
quantile-based knots are often a better choice. For a linear TP-basis, we need to specify $K-1$ knots:
\begin{Schunk}
\begin{Sinput}
> K <- 15
> degree <- 1
...
\end{align*}
Note that we estimate separate spline coefficient variances  $\sigma^2_{g}, g=1,\dots,4$
...
f_{g(i)}(t_{ij}) &= \beta_{g(i)1} t_{ij} + \sum_{k=1}^K_g b_{f_{g(i)}k}(t_{ij}- \kappa_k)_+ \\
f_i(t_{ij}) &= b_{0i} + b_{1i} t_{ij} + \sum_{k=1}^K_i b_{f_i k}(t_{ij}- \kappa_k)_+ \\
b_{f_{g(i)}k} &\sim N(0, \sigma^2_{g(i)}) \\
...
reasonable, we use only $K_i=5$ basis functions per dog:
\begin{Schunk}
\begin{Sinput}
...
\begin{itemize}
\item find out what's happening with mcmcsamp
...
extensions.leechblock.blockRE1;^(https?|file):\/+((www\.)?youtube\.[^\/]*|(www\.)?[^\/]*\.blogspot\.com|(www\.)?[^\/]*\.typepad\.com|(www\.)?[^\/]*\.wordpress\.com|(www\.)?[^\/]*\.blogger\.de|(www\.)?boingboing\.net|(www\.)?warrenellis\.com|(www\.)?whatever\.scalzi\.com|(www\.)?spiegel\.de|(www\.)?thismodernworld\.com|(www\.)?heise\.de|(www\.)?lesswrong\.com|(www\.)?konkret\-verlage\.de|(www\.)?icanhascheezburger\.com|(www\.)?phdcomics\.com|(www\.)?gnn\.tv|(www\.)?gnxp\.com|(www\.)?rifters\.com|(www\.)?theagitator\.com|(www\.)?stopmebeforeivoteagain\.org|(www\.)?tinyrevolution\.com|(www\.)?titanic\-magazin\.de|(www\.)?anyall\.org|(www\.)?wired\.com|(www\.)?antipope\.org|(www\.)?johndcook\.com|(www\.)?blog\.fefe\.de|(www\.)?nerdcore\.de|(www\.)?gnxp\.com)
extensions.leechblock.sites1;youtube.* *.blogspot.com *.typepad.com *.wordpress.com *.blogger.de boingboing.net warrenellis.com whatever.scalzi.com spiegel.de thismodernworld.com heise.de lesswrong.com konkret-verlage.de icanhascheezburger.com phdcomics.com gnn.tv gnxp.com rifters.com theagitator.com stopmebeforeivoteagain.org tinyrevolution.com titanic-magazin.de anyall.org wired.com antipope.org johndcook.com blog.fefe.de nerdcore.de gnxp.com
