...
a constant, linear, or a quadratic polynomial). %%%TRUE?
...
of the smooth terms will contain an intercept column. They are removed in order %%% REWRITE THIS!!
...
x.fx2} to \code{x.fxp}. We see that $\hat \sigma^2_f \approx$\Sexpr{round(unlist(VarCorr(u2)),2)}.
...
d3 <- amer(y ~ -1+ group + tp(time, k=5, by=dog, allPen=T, diag=T) + tp(time, by=group), data=dog)
@
